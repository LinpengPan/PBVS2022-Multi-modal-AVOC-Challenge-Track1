...
def _get_dataset_stats(
    ...
    if name == 'CIFAR10':
        ...
        mean = np.array([0.406796])
        std = np.array([0.134280])
    elif name == 'EO':
        mean = np.array([0.291819])
        std = np.array([0.148282])
    else:
        raise ValueError()
    return mean, std
...
def create_transform(config: yacs.config.CfgNode, is_train: bool) -> Callable:
    ...
    elif config.model.type == 'sar10':
        return create_sar10_transform(config, is_train)
    else:
        raise ValueError

def create_sar10_transform(config: yacs.config.CfgNode,
                           is_train: bool) -> Callable:
    mean, std = _get_dataset_stats(config)

    if is_train:
        transforms = []
        transforms.append(torchvision.transforms.Resize([56, 56]))
        # transforms.append(Filter())
        # if config.augmentation.use_filter:
        #     transforms.append(Filter())
