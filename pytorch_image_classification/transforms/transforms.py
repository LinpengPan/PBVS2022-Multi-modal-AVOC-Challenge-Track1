...
class RandomRotation:
    def __init__(self, config: yacs.config.CfgNode):
       self.angles = config.dataset.rotation_angles
    def __call__(self, data: PIL.Image.Image) -> PIL.Image.Image:
        angle = random.choice(self.angles)
        return TF.rotate(data, angle)
...
class Normalize:
    ...
    def __call__(self, image: PIL.Image.Image) -> np.ndarray:
        ...
        image = (image - self.mean) / self.std#train open
        return image
...
class Filter:
    def __init__(self):
        self.filter = PIL.ImageFilter.GaussianBlur(radius=1)

    def __call__(self, data: PIL.Image.Image) -> PIL.Image.Image:
        return data.filter(self.filter)
