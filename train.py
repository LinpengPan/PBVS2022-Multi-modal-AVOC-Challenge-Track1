...
def load_config():
    ...
    parser.add_argument('--config', type=str, default="./configs/sar10/shake_shake.yaml")
    ...
    config.frozen = args.frozen  # 最后更新确保不会被配置文件覆盖
    config.freeze()
...
def train(epoch, config, model, optimizer, scheduler, loss_func, train_loader,
    ...
    for step, (data, targets) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        if config.frozen:
            freeze_index = [0, 1, 2, 3, 4]
            freeze_para.set_freeze_by_idxs(model, freeze_index)
            optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=config.train.base_lr,
...
def validate(epoch, config, model, loss_func, val_loader, logger,
    ...
    with torch.no_grad():
        ...
        elapsed = time.time() - start
        logger.info(f'Elapsed {elapsed:.2f}')
        # with open('/media/disk1/ligongzhe/pytorch-cla/experiments/sar2022/plp/exp01/value.txt', 'a') as f:
        #     f.write(f'Epoch {epoch} ')
        #     f.write(f'loss {loss_meter.avg:.4f} ')
        #     f.write(f'acc@1 {acc1_meter.avg:.4f} ')
        #     f.write(f'acc@5 {acc5_meter.avg:.4f}')
        #     f.write('\n')
    ...
    if get_rank() == 0:
        ...
        # with open('/media/disk1/lixiaohan/pytorch-cla/experiments/cifar10/tzw/exp09/value_test.txt', 'a') as f:
        #     f.write(f'Epoch {epoch} ')
        #     f.write(f'loss {loss_meter.avg:.4f} ')
        #     f.write(f'acc@1 {acc1_meter.avg:.4f} ')
        #     f.write(f'acc@5 {acc5_meter.avg:.4f}')
